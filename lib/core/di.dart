...
Future<void> configureDependencies() async {
  ...
  dio.interceptors.add(
    InterceptorsWrapper(
      onRequest: (options, handler) async {
        ...
        final isCustomerRequest = options.path.startsWith('/customer') ||
        ...
        if (isCustomerRequest) {
          ...
          print('🔍 [INTERCEPTOR] Usando token de CUSTOMER para: ${options.path}');
        } else {
        ...
        if (token != null) {
          ...
          print('⚠️ [INTERCEPTOR] Nenhum token disponível para: ${options.path}');
        }
        ...
        // ✅ CORREÇÃO: Configura sendTimeout apenas para requisições com body (POST, PUT, PATCH)
        // Isso evita problemas no Web com requisições GET
      ...
      onError: (DioException e, handler) async {
        ...
        if (e.response?.statusCode == 401) {
          ...
          // ✅ CORREÇÃO CRÍTICA: Detecta se é requisição de CUSTOMER ou MENU
          final isCustomerRequest = e.requestOptions.path.startsWith('/customer') ||
  ...
  getIt.registerLazySingleton<OrdersCubit>(() => OrdersCubit(orderRepository: getIt<OrderRepository>()));
